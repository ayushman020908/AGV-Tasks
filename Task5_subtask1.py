"""
╔══════════════════════════════════════════════════════════════════════════════╗
║   AMCL FINAL — ACES Dataset                                                  ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                              ║
║  ANSWER TO YOUR QUESTIONS                                                    ║
║  ─────────────────────────────────────────────────────────────────────────  ║
║                                                                              ║
║  Q1: Are we using Map.png or the laser map?                                  ║
║      We MUST use the laser-built map. Here is WHY:                           ║
║                                                                              ║
║      map_no_green.png has corridors going N/S/E/W (axis-aligned).           ║
║      The ODOM data has the robot at -90° at start, and the corridors in     ║
║      the data go at ~35° to the ODOM axes. These are DIFFERENT coordinate   ║
║      frames. To use map_no_green.png you would first have to find the        ║
║      exact rotation+scale+translation that maps ODOM world coords to the    ║
║      image's pixel coords — and that cannot be done reliably without a       ║
║      known ground-truth reference point.                                     ║
║                                                                              ║
║      The laser-built map IS the building as seen in ODOM coordinates.       ║
║      The "rotation" you see (~35°) is REAL — the building is physically      ║
║      oriented at ~35° relative to the robot's starting heading.             ║
║                                                                              ║
║  Q2: The laser map has errors — can we make it cleaner?                      ║
║      YES. The white streaks (ray artifacts) are the main error.              ║
║      They appear because laser beams that exit through doorways or          ║
║      at range-limit get counted as "free space" past the building.           ║
║      Fix: keep only the LARGEST connected free-space component.              ║
║      One call to connectedComponentsWithStats() removes ALL 10,000+         ║
║      artifact streaks instantly, leaving only the true building interior.    ║
║                                                                              ║
║  ROOT BUGS FIXED (from previous runs)                                        ║
║  ─────────────────────────────────────────────────────────────────────────  ║
║                                                                              ║
║  BUG 1 — BIMODAL MEAN POSE (primary cause of diagonal jumps)                ║
║    Weighted mean of two clusters = point between them = on a wall.          ║
║    FIX: best_cluster_pose() — centroid of highest-weight cluster only.      ║
║                                                                              ║
║  BUG 2 — KLD COLLAPSES TO N=500 INSTANTLY                                  ║
║    KLD_BIN_XY=0.20m → only 2-5 bins → N_KLD formula → 500 every time.     ║
║    500 particles can't track 325m of symmetric corridors.                   ║
║    FIX: KLD_BIN_XY=0.50, KLD_BIN_TH=0.30, N_MIN=3000.                     ║
║                                                                              ║
║  BUG 3 — ANGLE OFFSET (one corridor wrong direction)                        ║
║    60 beams all from one arc → can't distinguish θ from θ+π in corridor.   ║
║    FIX: 4-quadrant beam selection — 22 beams from each of 4 quadrants.     ║
║                                                                              ║
╚══════════════════════════════════════════════════════════════════════════════╝
"""
